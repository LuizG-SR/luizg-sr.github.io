    ...
    if (mouseParticle.x && mouseParticle.y) {
      particles.forEach((p) => {
        ...
        if (dist < mouseConnectionRadius) {
          ctx.strokeStyle = `rgba(254, 200, 112, ${
            0.3 * (1 - dist / mouseConnectionRadius)
          })`; // Cor #FEC870 com opacidade
          ctx.beginPath();
...
function toggleMenu() {
  ...
  console.log('Menu toggled'); // Verificação para garantir que o código está sendo executado
}
